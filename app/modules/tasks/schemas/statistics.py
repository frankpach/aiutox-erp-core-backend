...
class TasksStatisticsResponse(BaseModel):
    ...
    by_custom_state: dict[str, int] = Field(..., description="Tasks grouped by custom state")
...
class CustomStateMetrics(BaseModel):
    ...
    avg_time_in_state_hours: float | None = Field(..., description="Average time in state (hours)")
...
class TasksOverviewResponse(BaseModel):
    ...
    custom_states: list[CustomStateMetrics] = Field(..., description="Custom state metrics")
...
class TasksByPriorityResponse(BaseModel):
    ...
    priority_distribution: dict[str, float] = Field(..., description="Priority distribution percentages")
...
class TaskCompletionTimeline(BaseModel):
    ...
    completion_time_hours: float | None = Field(None, description="Time to complete (hours)")
...
class TaskCompletionTimelineResponse(BaseModel):
    ...
    period: str = Field(..., description="Analysis period")
    timeline: list[TaskCompletionTimeline] = Field(..., description="Completion timeline")
    avg_completion_time: float | None = Field(None, description="Average completion time (hours)")
...
class StatisticsRequest(BaseModel):
    ...
    include_custom_states: bool = Field(True, description="Include custom state metrics")
