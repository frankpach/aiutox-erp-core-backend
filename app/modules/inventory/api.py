...
async def list_locations(
    ...
    locations = service.list_locations(
        current_user.tenant_id, warehouse_id=warehouse_id, skip=skip, limit=page_size
    )
    total = len(locations)
    total_pages = (total + page_size - 1) // page_size if total > 0 else 0
    return StandardListResponse(
...
async def create_location(
    ...
    location = service.create_location(
        current_user.tenant_id, warehouse_id, payload.model_dump()
    )
...
async def list_stock_moves(
    ...
    skip = (page - 1) * page_size
    moves = service.list_stock_moves(
        current_user.tenant_id, product_id=product_id, skip=skip, limit=page_size
    )
    total = len(moves)
    total_pages = (total + page_size - 1) // page_size if total > 0 else 0
    return StandardListResponse(
...
async def create_stock_move(
    ...
    current_user: Annotated[
        User, Depends(require_permission("inventory.adjust_stock"))
    ],
    service: Annotated[InventoryService, Depends(get_inventory_service)],
