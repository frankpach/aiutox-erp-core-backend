...
class InventoryService:
  ...
  def list_warehouses(self, tenant_id: UUID, skip: int = 0, limit: int = 100):
    return self.repository.list_warehouses(tenant_id=tenant_id, skip=skip, limit=limit)

  def get_warehouse(self, tenant_id: UUID, warehouse_id: UUID):
    warehouse = self.repository.get_warehouse(warehouse_id=warehouse_id, tenant_id=tenant_id)
    if not warehouse:
  ...
  def create_location(self, tenant_id: UUID, warehouse_id: UUID, data: dict):
    ...
    return self.repository.create_location({**data, "tenant_id": tenant_id, "warehouse_id": warehouse_id})

  def list_locations(self, tenant_id: UUID, warehouse_id: UUID | None = None, skip: int = 0, limit: int = 100):
    return self.repository.list_locations(tenant_id=tenant_id, warehouse_id=warehouse_id, skip=skip, limit=limit)

  def get_location(self, tenant_id: UUID, location_id: UUID):
    location = self.repository.get_location(location_id=location_id, tenant_id=tenant_id)
    if not location:
  ...
  def create_stock_move(self, tenant_id: UUID, user_id: UUID, data: dict):
    ...
    return self.repository.create_stock_move({**data, "tenant_id": tenant_id, "created_by": user_id})

  def list_stock_moves(self, tenant_id: UUID, product_id: UUID | None = None, skip: int = 0, limit: int = 100):
    return self.repository.list_stock_moves(tenant_id=tenant_id, product_id=product_id, skip=skip, limit=limit)
