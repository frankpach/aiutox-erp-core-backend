...
class S3ConfigResponse(BaseModel):
    ...
    secret_access_key: str = Field(
        default="***", description="Secret is never returned"
    )
    region: str
...
class LocalConfigBase(BaseModel):
    ...
    base_path: str = Field(
        default="./storage", description="Base path for local storage"
    )
...
class StorageConfigResponse(BaseModel):
    ...
    s3: S3ConfigResponse | None = Field(
        None, description="S3 configuration (if applicable)"
    )
    local: LocalConfigBase | None = Field(
        None, description="Local storage configuration (if applicable)"
    )
...
class StorageConfigUpdate(BaseModel):
    ...
    backend: str = Field(..., description="Storage backend: 'local', 's3', or 'hybrid'")
    s3: dict[str, Any] | None = Field(
        None, description="S3 configuration (required if backend is 's3' or 'hybrid')"
    )
    local: dict[str, Any] | None = Field(
        None,
        description="Local storage configuration (required if backend is 'local' or 'hybrid')",
    )
...
class StorageStatsResponse(BaseModel):
    ...
    mime_distribution: dict[str, int] = Field(
        default_factory=dict, description="Distribution of files by MIME type"
    )
    entity_distribution: dict[str, int] = Field(
        default_factory=dict, description="Distribution of files by entity type"
    )
    files_with_permissions: int = Field(
        ..., description="Number of files with permissions assigned"
    )
    folders_with_permissions: int = Field(
        ..., description="Number of folders with permissions assigned"
    )
    file_permission_distribution: dict[str, int] = Field(
        default_factory=dict,
        description="Distribution of file permissions by target type",
    )
    folder_permission_distribution: dict[str, int] = Field(
        default_factory=dict,
        description="Distribution of folder permissions by target type",
    )
...
class FileLimitsResponse(BaseModel):
    ...
    allowed_mime_types: list[str] = Field(
        default_factory=list, description="Allowed MIME types (empty = all allowed)"
    )
    blocked_mime_types: list[str] = Field(
        default_factory=list, description="Blocked MIME types"
    )
    max_versions_per_file: int = Field(..., description="Maximum versions per file")
    retention_days: int | None = Field(
        None, description="File retention period in days (None = no limit)"
    )
...
class FileLimitsUpdate(BaseModel):
    ...
    allowed_mime_types: list[str] | None = Field(
        None, description="Allowed MIME types (empty = all allowed)"
    )
    blocked_mime_types: list[str] | None = Field(None, description="Blocked MIME types")
    max_versions_per_file: int | None = Field(
        None, description="Maximum versions per file"
    )
    retention_days: int | None = Field(
        None, description="File retention period in days (None = no limit)"
    )
...
class ThumbnailConfigUpdate(BaseModel):
    ...
    default_width: int | None = Field(
        None, description="Default thumbnail width in pixels"
    )
    default_height: int | None = Field(
        None, description="Default thumbnail height in pixels"
    )
    quality: int | None = Field(None, description="JPEG quality (1-100)")
    cache_enabled: bool | None = Field(
        None, description="Whether thumbnail cache is enabled"
    )
