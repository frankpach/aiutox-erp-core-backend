...
class BulkTaskService:
        ...
        try:
            ...
            for task in tasks:
                try:
                    ...
                    updated_task = self.repository.update_task(task.id, {
                        "status": new_status,
                        "completed_at": task.completed_at if new_status == "done" else None,
                    ...
                    await self.cache_invalidation.invalidate_on_status_change(
                        tenant_id, task.id, old_status, new_status, user_id,
            ...
            for task in tasks:
                try:
                    ...
                    await self.cache_invalidation.invalidate_on_task_assign(
                        tenant_id, task.id, old_assigned_to_id, assigned_to_id,
            ...
            for task in tasks:
                try:
                    ...
                    await self.cache_invalidation.invalidate_on_task_update(
                        tenant_id, task.id, task.assigned_to_id, task.assigned_to_id,
            ...
            for task in tasks:
                try:
                    ...
                    await self.cache_invalidation.invalidate_on_task_delete(
                        tenant_id, task.id, task.assigned_to_id,
                        task.created_by_id, user_id
                    )
    ...
    def get_operation_summary(self, operation: BulkTaskOperation) -> dict:
        ...
        return {
            ...
            "success_rate": len(operation.results) / len(operation.task_ids) * 100 if operation.task_ids else 0,
