...
def require_task_permission(
    permission: TaskPermission, task_id_param: str | None = None
):
    ...
    def permission_dependency(
        ...
        if task:
            ...
            permission_methods = {
                ...
                TaskPermission.ASSIGN: lambda t, **kwargs: checker.can_assign_task(
                    t, kwargs.get("target_user_id")
                ),
                TaskPermission.CHANGE_STATUS: lambda t, **kwargs: checker.can_change_task_status(
                    t, kwargs.get("new_status", "")
                ),
...
CalendarSourcesManageUser = Annotated[
    User, Depends(require_calendar_sources_manage_permission)
]
