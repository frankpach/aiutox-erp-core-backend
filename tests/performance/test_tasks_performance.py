...
class TestTasksPerformance:
    ...
    @pytest.mark.asyncio
    async def test_create_task_performance(
        self,
        db_session,
        test_user,
        test_tenant
        ...
        assert elapsed_time < 100, f"Create task tomó {elapsed_time:.2f}ms (objetivo: <100ms)"
    ...
    async def test_list_tasks_performance(
        ...
        assert len(tasks) > 0
        assert elapsed_time < 100, f"List tasks tomó {elapsed_time:.2f}ms (objetivo: <100ms)"

    @pytest.mark.asyncio
    async def test_get_task_by_id_performance(
        self,
        db_session,
        test_user,
        test_tenant
        ...
        assert elapsed_time < 50, f"Get task by ID tomó {elapsed_time:.2f}ms (objetivo: <50ms)"

    @pytest.mark.asyncio
    async def test_update_task_performance(
        self,
        db_session,
        test_user,
        test_tenant
        ...
        assert elapsed_time < 100, f"Update task tomó {elapsed_time:.2f}ms (objetivo: <100ms)"
    ...
    async def test_create_task_from_template_performance(
        ...
        assert task.id is not None
        assert elapsed_time < 150, f"Create from template tomó {elapsed_time:.2f}ms (objetivo: <150ms)"
    ...
    async def test_bulk_create_performance(
        ...
        assert elapsed_time < 5.0, f"Bulk create tomó {elapsed_time:.2f}s (objetivo: <5s)"

        # Calcular tiempo promedio por tarea
        avg_time = (elapsed_time / 100) * 1000  # ms
        assert avg_time < 50, f"Tiempo promedio por tarea: {avg_time:.2f}ms (objetivo: <50ms)"
    ...
    async def test_query_with_filters_performance(
        ...
        assert len(tasks) > 0
        assert elapsed_time < 100, f"Query con filtros tomó {elapsed_time:.2f}ms (objetivo: <100ms)"
    ...
    async def test_checklist_operations_performance(
        ...
        assert elapsed_time < 100, f"Agregar checklist items tomó {elapsed_time:.2f}ms (objetivo: <100ms)"
