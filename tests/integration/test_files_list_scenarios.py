...
def test_list_files_empty_database(client, test_user, db_session):
    ...
    response = client.get("/api/v1/files?page=1&page_size=20", headers=headers)
...
def test_list_files_without_permissions(client, test_user, db_session):
...
def test_list_files_with_deleted_files(client, test_user, db_session):
...
def test_list_files_different_folders(client, test_user, db_session):
    ...
    response = client.get(f"/api/v1/files?folder_id={folder1.id}", headers=headers)
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.get(f"/api/v1/files?folder_id={folder2.id}", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data["data"]) == 1
...
def test_list_files_with_pagination(client, test_user, db_session):
    ...
    response = client.get("/api/v1/files?page=1&page_size=2", headers=headers)
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.get("/api/v1/files?page=2&page_size=2", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data["data"]) == 2
    ...
    response = client.get("/api/v1/files?page=3&page_size=2", headers=headers)
    assert response.status_code == 200
    data = response.json()
