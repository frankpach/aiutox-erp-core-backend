...
def test_get_stats_endpoint(client, auth_headers, mock_redis_client):
    ...
    with patch("app.api.v1.pubsub.get_redis_client", return_value=mock_redis_client):
        ...
        response = client.get("/api/v1/pubsub/stats", headers=auth_headers)

        # Note: This test may fail if pubsub.view permission doesn't exist
        # In that case, we need to add the permission to the system
        if response.status_code == 403:
            pytest.skip("pubsub.view permission not configured")
...
def test_list_failed_events_endpoint(client, auth_headers, mock_redis_client):
    ...
    with patch("app.api.v1.pubsub.get_redis_client", return_value=mock_redis_client):
        ...
        response = client.get("/api/v1/pubsub/failed", headers=auth_headers)
...
def test_get_stream_info_endpoint(client, auth_headers, mock_redis_client):
