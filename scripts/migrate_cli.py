...
def handle_migrate(manager: MigrationManager, reporter: MigrationReporter) -> None:
    ...
    if status.pending:
        ...
        for migration in status.pending:
            console.print(f"  • {migration.revision} - {migration.file}")
...
def handle_fresh(manager: MigrationManager, reporter: MigrationReporter) -> None:
    ...
    console.print("\n[bold red]⚠ WARNING: This will drop ALL tables![/bold red]")
    questions = [
        inquirer.Confirm(
            "confirm",
...
def handle_refresh(manager: MigrationManager, reporter: MigrationReporter) -> None:
    ...
    console.print("\n[bold yellow]⚠ This will rollback all migrations and re-apply them[/bold yellow]")
    questions = [
        inquirer.Confirm(
            "confirm",
